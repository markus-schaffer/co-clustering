funLBM_main <- function(X, K, L, maxit = 100, burn = 50, basis.name = 'fourier', nbasis = 15, gibbs.it = 5, display = FALSE, init = 'kmeans', ...) {
  ...
  if (init == 'funFEM') {
    ...
    cat('Initialization: random...\n');Z = t(rmultinom(N, 1, alpha))
  }
  Z = funLBM:::empty.class.check(Z)
  if (display) {
    cat('Z:'); print(colSums(Z))
  }
  ...
  if (init == 'funFEM') {
    ...
    cls = kmeans(t(apply(X_tot, 2, cbind)), L)$cluster; W = funLBM:::dummy(cls, L)
  } else {
    W = t(rmultinom(p, 1, beta))
  }
  W = funLBM:::empty.class.check(W)
  if (display) {
    cat('W:'); print(colSums(W))
  }
  Q = rep(list(list()), K)
  ...
  for (it in 1:maxit) {
    ...
    for (k in 1:K) {
      for (l in 1:L) {
        if (sum(W[, l]) == 1 | sum(Z[, k]) == 1) {
          x = C_tot[Z[, k] == 1, W[, l] == 1, ]
        } else {
          x = apply(C_tot[Z[, k] == 1, W[, l] == 1, ], 3, cbind)
        }
        ...
        dc = tryCatch(funLBM:::mypca.fd(obj), error = function(e) {
          stop("One class is empty, re-run the algorithm")
        })
        d[k, l] = funLBM:::cattell(dc$values)
    ...
    for (r in 1:gibbs.it) {
      # Z part
      Pz = funLBM:::estep.Z(C_tot, alpha, beta, mu, a, b, d, Q, W, Wmat)

      Z = tryCatch(t(apply(Pz, 1, function(x) {
        rmultinom(1, 1, x)
      })), error = function(e) {
        stop("One class is empty, re-run the algorithm")
      })
      Z = funLBM:::empty.class.check(Z, Pz)
      if (display) {
        cat('Z:'); print(colSums(Z))
      }

      # W part
      Pw = funLBM:::estep.W(C_tot, alpha, beta, mu, a, b, d, Q, Z, Wmat)
      W = tryCatch(t(apply(Pw, 1, function(x) {
        rmultinom(1, 1, x)
      })), error = function(e) {
        stop("One class is empty, re-run the algorithm")
      })
      Z = funLBM:::empty.class.check(Z, Pz)
      W = funLBM:::empty.class.check(W, Pw)
      if (display) {
        cat('W:'); print(colSums(W))
      }
    ...
    lik[it] = funLBM:::compute.CompleteLikelihood(C_tot, alpha, beta, mu, a, b, d, Q, Z, W, Wmat)
    Alphas[it, ] = alpha; Betas[it, ] = beta
    Zs[, it] = max.col(Z); Ws[, it] = max.col(W)

    # Test for early ending
    # Changed to 10 iterations (from 5)
    if (burn > 10 & it > burn) if (sum(abs(diff(lik[it:(it - 10)]))) < 1e-6) {
      burn = it - 10
      break
    }
  }
  ...
  Z = funLBM:::dummy(apply(Zs[, burn:it], 1, funLBM:::Mode), K)
  Z = funLBM:::empty.class.check(Z, Pz)
  W = funLBM:::dummy(apply(Ws[, burn:it], 1, funLBM:::Mode), L)
  W = funLBM:::empty.class.check(W, Pw)
  for (k in 1:K) {
    for (l in 1:L) {
      if (sum(W[, l]) == 1 | sum(Z[, k]) == 1) {
  ...
  crit = funLBM:::compute.CompleteLikelihood(C_tot, alpha, beta, mu, a, b, d, Q, Z, W, Wmat) - (K - 1) / 2 * log(N) - (L - 1) / 2 * log(p) - nu / 2 * log(N * p)
  ...
  out = list(
    basisName = basis.name, nbasis = nbasis, T = T, K = K, L = L, prms = prms, Z = Z, W = W, Pz = Pz,
    row_clust = max.col(Z), col_clust = max.col(W), allPrms = allPrms, loglik = lik, icl = crit, it = it, burn = burn
  )
  class(out) = "funLBM"
  out
}
