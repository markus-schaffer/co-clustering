...
if (!dir.exists(file.path("data/06_vsurf"))) {
  dir.create(file.path("data/06_vsurf"))
}
...
# VSURF lead to problems if executed in a function
for (i in 1:length(data)) {
  for (j in 1:length(outer_folds)) {
    ...
    set.seed(123)
    vsurf <- VSURF(
      x = train_X, y = train_Y, parallel = TRUE, ncores = 50, verbose = FALSE,
    ...
    # Test statistics and confusion matrix
    test_mcc <- map_dbl(pred, ~ mcc_vec(truth = test_Y, estimate = .x))
    test_confuse_pred <- multi_class_cm(train_labels = pred$pred, truth_labels = test_Y)
    ...
    result <- data.table(
      ...
      "test_confuse_pred" = list(test_confuse_pred),
    ...
    saveRDS(result, paste0("data/06_vsurf/", i, "_", j, ".RDS"))
  }
}

# Collect result and delete the temp folder
files <- fs::dir_ls(path = "data/06_vsurf")
result_vsurf <- map(files, ~ readRDS(.x)) |> rbindlist()
saveRDS(result_vsurf, "data/06_vsurf.RDS")
unlink("data/06_vsurf", recursive = TRUE)

# result_vsurf <- readRDS("data/06_vsurf.RDS")
...
saveRDS(result_ranger, "data/06_ranger.RDS")
# result_ranger <- readRDS("data/06_ranger.RDS")
